function [ model_connect ] = connect_model1( model_sample, model_ether )
%Connect specimen model and vacuum model
%This is first step to check if a point from sample is in vasuum region,
%and vice versa
...
%close (h);
toc
%set model_connect.tetra_reg_ether all [] at beginning
  
%tmp_an=zeros(size(model_ether.t,2),1);
for ii=1:size(model_ether.t,2)
    model_connect.tetra_reg_ether{ii,1}=[];
end
...
for ii=1:size(model_sample.t,2)
    model_connect.tetra_reg_sample{ii,1}=[];
end
