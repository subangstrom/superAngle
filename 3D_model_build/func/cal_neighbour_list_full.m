function [model_mesh_nei]=cal_neighbour_list_full(model_mesh, chk)
%generate neighbour list (4 faces) of each tetrahedral 
%this version will save all intermediate results in the structure
...
for ii=1:l
    ...
    for jj=1:model_mesh_nei.tot_neighbour(ii,1)
        ...
        model_mesh_nei.nei_list_coplane.co_face{ii,jj}=[tmpa tmpb tmpc tmpd];
        if (model_mesh_nei.tot_neighbour(ii,1)<=3)
            for chk_face=1:4
                if (model_mesh_nei.nei_list_coplane.co_face{ii,jj}==face{chk_face,1})
                    face{chk_face,2}=0;
                end
            end
        end
    ...
    if (model_mesh_nei.tot_neighbour(ii,1)<=3)
        ...
        for chk_face=1:4
            if (face{chk_face,2}==1)
                t_num=t_num+1;
                model_mesh_nei.nei_list_coplane.surface{ii,t_num}=face{chk_face,1};
            end
        end
    end
...
for ii=1:l
    ...
    for jj=1:4
        ...
        for num=1:model_mesh_nei.tot_neighbour(ii,1);
            plane_comp=model_mesh_nei.nei_list_coplane.co_face{ii,num};
