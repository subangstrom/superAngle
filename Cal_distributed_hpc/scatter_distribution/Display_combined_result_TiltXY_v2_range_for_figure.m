...
%angle_range=10; %within deg  
angle_range=0:2:30; %within deg 
num_i=0;
...
if (length(angle_range)==1)
    ...
    for j=1:length(det_list)
        figure;imagesc(E_image_det_comb{j}, [0 15]);axis image;colorbar;
...
%plot line profile
    ...
    %set(gca,'YScale','log');
    legend('Det 1','Det 2','Det 3','Det 4','Det 1+3','Sum','Location','northwest')
...
%% save data for the total figure
%compare det 1, det 3, det 1+3, det sum
%add to a output matrix
x_E18_grid{1}=plot_x;
y_E18_grid{1}=plot_y_invid{1}; %det 1
y_E18_grid{2}=plot_y_invid{3}; %det 3
y_E18_grid{3}=plot_y_det_comb{1}; %det 1+3
y_E18_grid{4}=plot_y_sum; %det sum
save('out_E18_grid.mat','x_E18_grid','y_E18_grid');