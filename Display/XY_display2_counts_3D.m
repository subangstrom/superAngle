function [  ] = XY_display2_counts_3D( Tilt_map, chk_print, Detector, sample_para, search_Deg_2D, d_Deg_2D )
...
A_map_counts=Tilt_map.A_map_counts;
B_map_counts=Tilt_map.B_map_counts;
...
if (chk_print==1)
    ...
    print(['map_2D_',title_name],'-depsc','-tiff');
end
...
for i=1:tot_Det_num
    figure;
    set(gca,'FontSize',15)
    imagesc (image_range, -image_range, A_map_counts(:,:,i)./B_map_counts(:,:,i)), axis image;
    ...
    colormap (maptype);
    hold on;
    [C,h]=contour (image_range, -image_range, A_map_counts(:,:,i)./B_map_counts(:,:,i),'k');
